...
def train(args, train_dataset, model, tokenizer) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            num_correct += (preds == out_label_ids).sum()
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if args.model_name_or_path:
        model_name = args.model_name_or_path.split("/")
    else:
        model_name = 'roberta'
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            ...
            list(filter(None, model_name)).pop(),
...
def main():
    ...
    pytorch_total_params = sum(p.detach().numel() for p in model.parameters())
    pytorch_total_params_trainable = sum(p.detach().numel() for p in model.parameters() if p.requires_grad)
    ...
    if args.save_lr_accu:
        ...
        with open('../example/pytorch/accuracy_and_lr_record/glue_accuracy/SST2/'+compressor_name+args.model_type+'-'+curr_time+'.pkl', 'wb') as pkl_file:
